...
<body class="p-4">
  ...
  <form method="post" action="/admin/room/create" enctype="multipart/form-data">
    ...
    <div class="form-group">
      ...
      <textarea class="form-control" id="seo_tagInput" rows="3" name="seo_tag">{{ old('seo_tag') }}</textarea>
      <br>
      <p style="color:red;">{{$errors->first('seo_tag')}}</p>
    </div>
    <div class="form-group">
      <label for="breadthInput">部屋の広さ</label>
      @foreach($breadthChecks as $breadthCheck)
      <input type="checkbox" name="breadthCheck" value="{{ $breadthCheck['breadth_id'] }}" @if (old('breadth') && array_search($breadthCheck['breadth_id'], explode(",", old('breadth')))!==false) checked @endif> {{ $breadthCheck['breadth_name'] }}
      @endforeach
      <br>
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="type_tagInput" name="type_tag" value="{{ old('type_tag') }}">
      <br>
      <p style="color:red;">{{$errors->first('type_tag')}}</p>
    </div>
    <div class="form-group">
      <label for="publishInput">公開する</label>
      <input type="checkbox" name="publish" value="1" @if (old('publish') == 1) checked @endif>
    </div>
