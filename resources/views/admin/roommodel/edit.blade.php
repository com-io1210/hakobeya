...
<html lang="ja">
  ...
  <body class="p-3">
    ...
    <form method="post" action="/admin/roommodel/edit" enctype="multipart/form-data">
      {{ csrf_field() }}
      <input type="hidden" class="form-control" name="id" value="{{ $roommodel->id }}">
      <label for="room_idInput">部屋選択</label>
      <select name="room_id" id="room_idInput">
        @foreach($rooms as $room)
        <option value={{ $room->id }}@if ($room->id === $roommodel->id) selected @endif>{{ $room->title }}</option>
        @endforeach
      </select>
      <br>
    ...
    <div class="form-group">
      ...
      <br>
      <p style="color:red;">{{$errors->first('model')}}</p>s
    </div>
    <div class="form-group">
