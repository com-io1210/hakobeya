  ...
  <form method="post" action="/admin/furnituremodel/edit" enctype="multipart/form-data">
    ...
    <div class="form-group">
      ...
      @foreach($genleSelects as $genleSelect)
        <option value="{{ $genleSelect['genle_id'] }}" @if($furnituremodel->genle === $genleSelect['genle_id']) selecded @endif>{{ $genleSelect['genle_name'] }}</option>
        @endforeach
      </select>
    ...
    <div class="form-group">
      ...
      <img src="{{$furnituremodel->image}}">
      <p>{{$furnituremodel->image}}</p>
      <input type="file" class="form-control" id="imageInput" name="image" value="{{ $furnituremodel->image }}">
    </div>
